...
public class Manage extends Form {
    ...
    ReclamationCategory selectedtAnimalsCategory = null;
    
    
    Button manageButton;

    Form previous;
    private ReclamationCategory selectedAnimalsCategory;

    public Manage(Form previous) {
        super(ShowAll.currentAnimal == null ?  "Ajouter" :  "Modifier", new BoxLayout(BoxLayout.Y_AXIS));
        this.previous = previous;

        currentAnimals = ShowAll.currentAnimal;
...
//            AnimalsCategoryPC.getPicker().setSelectedString((String) currentAnimals.getAnimalsCategory().getEspece());
        ...
        if (currentAnimals == null) {
            manageButton.addActionListener(action -> {
                if (controleDeSaisie()) {
                    ...
                    if (responseCode == 200) {
                        AlertUtils.makeNotification("Animal ajouté avec succes");
                        showBackAndRefresh();
                    } else {
                        Dialog.show("Erreur", "Erreur d'ajout  Animals. Code d'erreur : " + responseCode, new Command("Ok"));
                    }
                }
            });
        } else {
            manageButton.addActionListener(action -> {
                if (controleDeSaisie()) {
                    ...
                    if (responseCode == 200) {
                        ...
                        Dialog.show("Erreur", "Erreur de modification  reclamation. Code d'erreur : " + responseCode, new Command("Ok"));
                    }
                }
            });
        }
    ...
    private boolean controleDeSaisie() {
        ...
        if (GenreAnimalTF.getText().equals("")) {
            Dialog.show("Avertissement", "GznreAnimal vide", new Command("Ok"));
            return false;
        }
        ...
        if (selectedAnimalsCategory == null) {
            Dialog.show("Avertissement", "Veuillez choisir un AnimalsCategory", new Command("Ok"));
            return false;
        }
